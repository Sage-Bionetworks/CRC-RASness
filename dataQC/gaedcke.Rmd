...
The first two singular values were not strongly associated with any clinical covariates and there appears to be no single outlier nor distinct group of outliers. The effects of age and gender were subsequently removed from the data by removing the gene-specific fitted values of a linear model with age and gender included. The first SVD was then used to collapse multiple probe to a single expression value per gene where necessary. SVD plots were repeated to ensure no artifacts were introduced in this process.
...
This final R object is an `ExpressionSet` containing gene-level expression values as well as clinical information on the samples within the `phenoData` slot of the object.
